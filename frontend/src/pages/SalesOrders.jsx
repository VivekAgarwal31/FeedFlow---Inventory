...
import { Plus, TrendingUp, Loader2, Trash2, Search, Eye, Edit, Download } from 'lucide-react'
import { salesOrderAPI, stockAPI, clientAPI } from '../lib/api'
import { downloadInvoiceByOrder, downloadBlob } from '../lib/paymentApi'
...
const SalesOrders = () => {
    ...
    const fetchData = async () => {
        try {
            setLoading(true)
            const [ordersResponse, stockResponse, clientsResponse] = await Promise.all([
                salesOrderAPI.getAll({ page: 1, limit: 100 }),
                stockAPI.getAll(),
                clientAPI.getAll()
            ])

            setSalesOrders(ordersResponse.data.salesOrders || [])
    ...
    const addItemToOrder = () => {
        ...
        if (existingItemIndex >= 0) {
            ...
            const newItem = {
                ...
                quantity: parseInt(currentItem.quantity),
                sellingPrice: parseFloat(currentItem.unitPrice),
                total: parseInt(currentItem.quantity) * parseFloat(currentItem.unitPrice)
            }
    ...
    const deleteOrder = async (orderId, clientName) => {
        if (!confirm(`Are you sure you want to delete this sales order for ${clientName}?`)) {
            return
        }
        ...
        try {
            ...
            toast({
                title: 'Error',
                description: error.response?.data?.message || 'Failed to delete sales order',
                variant: 'destructive'
            })
        }
    }
    ...
    const downloadInvoice = async (orderId, orderNumber) => {
        try {
            const blob = await downloadInvoiceByOrder(orderId)
    ...
    const paginatedOrders = salesOrders.slice(startIndex, startIndex + itemsPerPage)
    ...
    return (
        <div className="space-y-6">
            <div className="flex items-center justify-between">
                <div>
                    ...
                    <p className="text-muted-foreground mt-1">
                        Create and manage customer orders (no stock impact)
                    </p>
                </div>
                ...
                <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
                    ...
                    <DialogContent className="max-w-4xl max-h-[90vh] overflow-y-auto">
                        <DialogHeader>
                            ...
                            <DialogDescription>
                                Create an order without affecting stock. Delivery will be created separately.
                            </DialogDescription>
                        </DialogHeader>
                                    ...
                                    <CardContent>
                                        <Table>
                                            ...
                                            <TableBody>
                                                ...
                                                <TableRow>
                                                    <TableCell colSpan={2} className="font-medium">Items Subtotal</TableCell>
            ...
            {stockItems.length === 0 && (
                <Alert>
                    <AlertDescription>
                        No stock items available. Please add stock items first to create sales orders.
                    </AlertDescription>
                </Alert>
            )}
            ...
            <Card>
                <CardHeader>
                    ...
                    <CardDescription>
                        Customer orders awaiting delivery
                    </CardDescription>
                </CardHeader>
                <CardContent>
                                                ...
                                                <div className="flex flex-col gap-1">
                                                    ...
                                                    <Badge variant={getStatusBadgeVariant(order.orderStatus)} className="text-xs">
                                                        {order.orderStatus === 'completed' ? 'DELIVERED' :
                                                            order.orderStatus === 'partially_delivered' ? 'PARTIALLY DELIVERED' : 'NOT DELIVERED'}
                                                    </Badge>
                                                </div>
                                            </TableCell>
                                            <TableCell>
                                                <div className="flex gap-1">
                                                    ...
                                                    <Button
                                                        variant="ghost"
                                                        size="sm"
                                                        onClick={() => deleteOrder(order._id, order.clientName)}
                        ...
                        <div className="space-y-6">
                            ...
                            <div>
                                ...
                                <Table>
                                    ...
                                    <TableBody>
                                        ...
                                        {selectedOrder.wages > 0 && (
                                            <TableRow>
                                                <TableCell colSpan={5} className="font-medium">Wages</TableCell>
                                                <TableCell className="font-mono">{formatCurrency(selectedOrder.wages)}</TableCell>
                                            </TableRow>
                                        )}
                                        <TableRow>
                                            <TableCell colSpan={5} className="font-semibold">Total Amount</TableCell>
                                            <TableCell className="font-mono font-bold text-lg">{formatCurrency(selectedOrder.totalAmount)}</TableCell>
                                        </TableRow>
                                    </TableBody>
                                </Table>
                            </div>
                            ...
                            <div className="grid grid-cols-3 gap-4 p-4 bg-muted/30 rounded-lg">
                                <div>
                                    ...
                                    <p className="font-mono font-bold">{formatCurrency(selectedOrder.totalAmount)}</p>
                                </div>
                                <div>
                                    <Label className="text-muted-foreground">Amount Paid</Label>
                                    <p className="font-mono font-bold text-green-600">{formatCurrency(selectedOrder.amountPaid || 0)}</p>
                                </div>
                                <div>
                                    <Label className="text-muted-foreground">Amount Due</Label>
                                    <p className="font-mono font-bold text-orange-600">{formatCurrency(selectedOrder.amountDue || selectedOrder.totalAmount)}</p>
                                </div>
                            </div>
